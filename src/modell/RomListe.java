...
public class RomListe {
	...
	public static ArrayList<Rom> ledigeMedMinimumKapasitet(int kapasitet,
		...
		String slutta = "from_unixtime(" + ((int) (slutt.getTime() * .001))
				+ ")";
		return medSql("select id from rom where id NOT IN("
				+ "select r.id from rom as r,avtale as a where a.aktiv=1 and r.id=a.rom_id and r.kapasitet >= "
				+ kapasitet + " and ((" + starta + " <= a.start and a.start < "
				+ slutta + ") or (" + starta + " <= a.slutt and a.slutt < "
				+ slutta + ") or (" + starta + " <= a.start and a.slutt < "
				+ slutta + ") or (a.start <= " + starta + " and " + slutta
				+ " < a.slutt)));");
	}
